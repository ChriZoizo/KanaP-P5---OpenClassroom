...
function checkDataFromForm() {
 ...
 let adress = document.getElementById("address").value.toString()
  ...
  if (regExNamesChecker.test(contact["firstName"]) == true) {
    ...
    createObjectsToSend(firstName, lastName, adress, city, email, productsIds)
    console.log('##############################')
    cartList.forEach(function (product) {
...
function createObjectsToSend(firstName, lastName, adress, city, email, productsIds){
 let contact = new Contact(firstName, lastName, adress, city, email)
...
function displayConfirmationCode(confirmationCode){
